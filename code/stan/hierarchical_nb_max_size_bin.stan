...
parameters {
  /*
   * Across-site centers for the latent normal location and scale.
   */
  real alpha_log_mu;
  real alpha_log_sigma;

  /*
   * Site-level latent normal parameters, sampled directly on bounded
   * log scales.
   */
  vector<
    lower=log(1e-8),
  ...
  /*
   * Among-site variation in latent normal location and scale.
   */
  real<lower=1e-4, upper=2.5> tau_log_mu;
  real<lower=1e-4, upper=2.5> tau_log_sigma;

   /*
   * Position of the shared taxon-level endpoint between:
   *
   * max_bin_edge_taxon
   *
   * and
  ...
  /*
   * Non-centered hierarchy for expected event counts.
   */
...
model {
  /*
   * Across-site size hyperpriors, retained from the previous stable
   * hierarchical model.
   */
  alpha_log_mu ~
    student_t(
